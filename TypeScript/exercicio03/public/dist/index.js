const users = [];
async function getUserGithub(nameRepo) {
    try {
...
async function showInfoUser(name) {
    const user = users.find(u => u.login === name);
    if (!user) {
...
async function resolvePromisse(user) {
    const response = await fetch(user.repos_url);
    if (!response.ok) {
        throw new Error("Error na requisição!");
    }
    const data = await response.json();
    const repoFiltered = data.map((repo) => {
        return {
            name: repo.name,
...
async function getReposLength(user) {
    let count = 0;
    const response = await fetch(user.repos_url);
    const data = await response.json();
    count += data.length;
